...
export default function SpinToWin({ chance }: SpinToWinProps) {
  ...
  const spin = () => {
    ...
    const newAngle = angle - (1080 + randomOffset); // rotire la stânga (negativ)
    setAngle(newAngle);
  ...
  return (
    <div className="p-4 rounded flex flex-col text-center">
      <div className="relative w-64 h-64">
        ...
        {/* punct rotativ apropiat de cerc */}
        <div
      ...
      >
        ...
        <p className="ml-3">
          {spinning ? "Se învârte..." : "Demo Spin"}</p>
      </button>

      {result && <p className="mt-4 absolute bottom-[-5vh] text-center text-lg font-semibold">{result}</p>}
    </div>
  );
}
